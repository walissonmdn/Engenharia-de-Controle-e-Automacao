...
from kia.worksheets.models_worksheet import ModelsWorksheet
from kia.worksheets.results_worksheet import ResultsWorksheet
from kia.worksheets.stores_worksheet import StoresWorksheet
from kia.worksheets.fuels_worksheet import FuelsWorksheet
...
class Dealer:
    ...
    def fill_in_informations(self):
        ...
        # Opcional.
        if self.datapool_item["pintura"] != None:
            ...
            self.selenium.script(f"document.getElementById('vDESCRICAO').value = '{self.datapool_item["pintura"].replace(" ", "%")}'")
            self.selenium.script(f"document.getElementById('vSIGLA').value = '{self.datapool_item["sigla_da_pintura"]}'")
            time.sleep(0.5)
            ...
            try:
                ...
                self.results_worksheet.write(results_worksheet.OBSERVACAO_COLUMN,
                                             self.current_worksheet_row,
                                             "Opcional não encontrado.")
            finally:
                opcional = re.sub(r"\s+", " ", opcional) # Remove espaços a mais.
                if ((opcional.strip() == self.datapool_item["pintura"]) and
                    (sigla.strip() == self.datapool_item["sigla_da_pintura"])
                    ):
        ...
        # Modelo.
        model_code = self.models_worksheet.get_model_code(self.datapool_item["descricao_do_produto"],
                                                          self.datapool_item["codigo_do_modelo"])
        ...
        # Cor interna (Será sempre PRETO/CINZA para a KIA).
    ...
    def process_data(self):
        ...
        self.selenium.click("input#BTNPROCESSAR", wait_time=5)
