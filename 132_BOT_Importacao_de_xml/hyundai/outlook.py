...
class Outlook:
    ...
    def download_attachments(self):
        ...
        for _ in range(3):
            try:
                ...
                # Seleciona a caixa de entrada principal e impede que as mensagens de e-mail sejam 
                ...
                for message_index in messages_index[0].split():
                    ...
                    # Caso esteja especificado no assunto do e-mail que se trata de nota fiscal da 
                    # Hyundai, baixar o xml.
                    if "envio da nfe - emitente: hyundai" in unidecode(subject).lower():
                        for part in e_mail.walk():
                            ...
                            if ".xml" in attachment.lower():
                                ...
                                file_object = open(f"{DOCUMENTS_DOWNLOADS_FOLDER_PATH}\\{attachment}", "wb")
                                ...
                                self.log.info(message=f'Arquivo baixado: "{attachment}".')

                    elif "cancelamento da nfe - emitente: hyundai" in unidecode(subject).lower():
                        for part in e_mail.walk():
                            if part.get_content_maintype() == "multipart":
                                continue
                            if part.get("Content-Disposition") is None:
                            ...
                            if ".xml" in attachment.lower():
                                # Recebe arquivo para escrita e salva no computador.
                                file_object = open(f"{CANCELLED_DOCUMENTS_DOWNLOADS_FOLDER_PATH}\\{attachment}", "wb")
                                file_object.write(part.get_payload(decode = True))

                                # Permite marcar a mensagem como lida e marca.
                                mail.select(mailbox='"XML - Veiculos"', readonly=False)
                                mail.store(message_index, "+FLAGS", '\\Seen') # Marca como lida.
                                self.log.info(message=f'Nota cancelada: "{attachment}".')
                                self.log.info(message=f'Arquivo baixado: "{attachment}".')

                mail.close()
                mail.logout()
                self.log.info("A conexão com o outlook foi encerrada.")
                break
    ...
    def send_partial_report(self):
        ...
        recipients = ["gerentefaturamentobsb@gruposaga.com.br",
                      "nfebsb@gruposaga.com.br",
                      "lais.abarros@gruposaga.com.br",
                      "jhennyfer.smoreira@gruposaga.com.br"]
        
        # recipients = ["walisson.marcelo2000@gmail.com"]

        # Remetente, destinatário e Assunto do e-mail.
        message = MIMEMultipart()
        message["From"] = "Saga RPA <saga.rpa@gruposaga.com.br>"
        message["To"] = ", ".join(recipients)
        message["Subject"] = f"RPA - Importação de XML - HYUNDAI"

        # Mensagem (HTML).
        html_message = f"""
            <div>
                <p>Olá!</p>
                <p>Segue em anexo a planilha com os resultados da importação dos arquivos de veículos da HYUNDAI.</p>
